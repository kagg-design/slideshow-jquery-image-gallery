...
class SlideshowPluginGeneralSettings {
	...
	public static $settingsGroup = 'slideshow-jquery-image-gallery-general-settings';

	/** @var string $stylesheetLocation Stylesheet location setting */
	public static $stylesheetLocation = 'slideshow-jquery-image-gallery-stylesheet-location';

	/** @var string $enableLazyLoading Lazy loading setting */
	public static $enableLazyLoading = 'slideshow-jquery-image-gallery-enable-lazy-loading';
	...
	public static $defaultSettings = 'slideshow-jquery-image-gallery-default-settings';
	/** @var string $defaultStyleSettings */
	public static $defaultStyleSettings = 'slideshow-jquery-image-gallery-default-style-settings';

	/** @var string $customStyles List of pointers to custom style options */
	public static $customStyles = 'slideshow-jquery-image-gallery-custom-styles';
	...
	public static function init() {
		...
		if (
			isset( $_GET['post_type'] ) && 'slideshow' === $_GET['post_type'] &&
			isset( $_GET['page'] ) && 'general_settings' === $_GET['page']
		) {
		...
		// Add sub menu.
		add_action( 'admin_menu', [ __CLASS__, 'addSubMenuPage' ] );

		// Localize.
		add_action( 'admin_enqueue_scripts', [ __CLASS__, 'localizeScript' ], 11 );
	}
	...
	public static function registerSettings() {
		...
		register_setting(
			self::$settingsGroup,
			self::$capabilities['deleteSlideshows'],
			[ __CLASS__, 'saveCapabilities' ]
		);
		...
		register_setting( self::$settingsGroup, self::$customStyles, [ __CLASS__, 'saveCustomStyles' ] );
	}
	...
	public static function localizeScript() {
		...
		wp_localize_script(
			...
			[
				'data'         => [ 'customStylesKey' => self::$customStyles ],
				'localization' => [
	...
	public static function getStylesheetLocation() {
		return get_option( self::$stylesheetLocation, 'footer' );
	}
	...
	public static function getStylesheets( $withVersion = false, $separateDefaultFromCustom = false ) {
		...
		// Get custom styles.
		$customStyles = get_option( self::$customStyles, [] );
		...
		if ( $withVersion ) {
			foreach ( $customStyles as $customStylesKey => $customStylesName ) {
				...
				$customStyles[ $customStylesKey ] = [
					'name'    => $customStylesName,
					'version' => $customStylesVersion,
				];
			}
		}
	...
	public static function saveCapabilities( $capability ) {
		...
		foreach ( $wp_roles->roles as $roleSlug => $roleValues ) {
			...
			foreach ( self::$capabilities as $capabilitySlug ) {
				...
				if (
					( isset( $_POST[ $capabilitySlug ] ) && is_array( $_POST[ $capabilitySlug ] ) && array_key_exists( $roleSlug, $_POST[ $capabilitySlug ] ) ) ||
					'administrator' === $roleSlug
				) {
	 ...
	 * @since 2.1.23
	 */
	public static function saveCustomStyles( $customStyles ) {
		// Verify nonce.
		$nonce = isset( $_POST['_wpnonce'] ) ? $_POST['_wpnonce'] : '';
		...
		$oldCustomStyles = get_option( self::$customStyles, [] );
