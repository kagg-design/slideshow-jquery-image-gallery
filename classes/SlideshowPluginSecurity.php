			...
			foreach ( $allowedElements as $element => $attributes ) {
				...
				while ( ( $position = stripos( $text, $element, $position ) ) !== false ) {
					...
					if ( substr( $text, $position - strlen( $encodedOpeningTag ), strlen( $encodedOpeningTag ) ) == $encodedOpeningTag ) {
						...
						} elseif ( isset( $attributes['attributes'] ) && 'default' === $attributes['attributes'] ) {
						...
						foreach ( $elementAttributes as $attribute ) {
							...
							if ( ( $attributePosition = stripos( $tagText, $attributeOpener, $attributePosition ) ) !== false ) {
								$attributeClosingPosition = 0;

								if ( ( $attributeClosingPosition = stripos( $tagText, htmlspecialchars( '"' ), $attributePosition + strlen( $attributeOpener ) ) ) === false ) {
									continue;
								}
