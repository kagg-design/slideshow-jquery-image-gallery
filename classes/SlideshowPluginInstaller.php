...
';
		...
		$customStyles = get_option( $customStylesOptionsKey, [] );
	...
	private static function updateV2_2_12_to_V_2_2_16() {
		...
		if ( is_array( $slideshows ) &&
			...
			foreach ( $slideshows as $slideshow ) {
				...
				if ( isset( $settings['stretchImages'] ) ) {
					if ( 'true' === $settings['stretchImages'] ) {
	...
	private static function updateV2_1_23_to_V_2_2_0() {
		...
		if ( is_array( $slideshows ) &&
			...
			foreach ( $slideshows as $slideshow ) {
				// Get settings.
				$styleSettings = maybe_unserialize(
					get_post_meta(
						$slideshow->ID,
						'styleSettings',
						true
				...
				if (
					isset( $styleSettings['style'] ) &&
					'light' !== $styleSettings['style'] &&
					'dark' !== $styleSettings['style']
				) {
		...
		if ( is_array( $slideshows ) &&
			...
			foreach ( $slideshows as $slideshow ) {
				...
				if (
					...
					$styleSheets = get_option( $stylesKey, [] );
		...
		if (
			...
			foreach ( $slideshows as $slideshow ) {
				...
				foreach ( $settings as $key => $value ) {
					if ( substr( $key, 0, strlen( $settingsPrefix ) ) === $settingsPrefix ) {
						$newSettings[ substr( $key, strlen( $settingsPrefix ) ) ] = $value;
					} elseif ( substr( $key, 0, strlen( $stylePrefix ) ) === $stylePrefix ) {
						$styleSettings[ substr( $key, strlen( $stylePrefix ) ) ] = $value;
					} elseif ( substr( $key, 0, strlen( $slidePrefix ) ) === $slidePrefix ) {
		...
		foreach ( $posts as $post ) {
			...
			$data = [
				'style_style'               => [
					'select',
					'',
					$defaultData['style_style'],
					__( 'The style used for this slideshow', 'slideshow-jquery-image-gallery' ),
					[
						'light'  => __( 'Light', 'slideshow-jquery-image-gallery' ),
						'dark'   => __( 'Dark', 'slideshow-jquery-image-gallery' ),
						'custom' => __( 'Custom', 'slideshow-jquery-image-gallery' ),
					],
				],
				'style_custom'              => [
					'textarea',
					'',
					$defaultData['style_custom'],
					__( 'Custom style editor', 'slideshow-jquery-image-gallery' ),
					null,
					[ 'style_style', 'custom' ],
				],
				'setting_animation'         => [
					'select',
					'',
					$defaultData['setting_animation'],
					__( 'Animation used for transition between slides', 'slideshow-jquery-image-gallery' ),
					[
						'slide' => __( 'Slide', 'slideshow-jquery-image-gallery' ),
				...
				'setting_slideSpeed'        => [
					'text',
					'',
					$defaultData['setting_slideSpeed'],
					__( 'Number of seconds the slide takes to slide in', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Animation', 'slideshow-jquery-image-gallery' ),
				],
				'setting_descriptionSpeed'  => [
					'text',
					'',
					$defaultData['setting_descriptionSpeed'],
					__( 'Number of seconds the description takes to slide in', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Animation', 'slideshow-jquery-image-gallery' ),
				],
				'setting_intervalSpeed'     => [
					'text',
					'',
					$defaultData['setting_intervalSpeed'],
					__( 'Seconds between changing slides', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Animation', 'slideshow-jquery-image-gallery' ),
				],
				'setting_slidesPerView'     => [
					'text',
					'',
					$defaultData['setting_slidesPerView'],
					__( 'Number of slides to fit into one slide', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Display', 'slideshow-jquery-image-gallery' ),
				],
				'setting_width'             => [
					'text',
					'',
					$defaultData['setting_width'],
					__( 'Width of the slideshow, set to parent&#39;s width on 0', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Display', 'slideshow-jquery-image-gallery' ),
				],
				'setting_height'            => [
					'text',
					'',
					$defaultData['setting_height'],
					__( 'Height of the slideshow', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Display', 'slideshow-jquery-image-gallery' ),
				],
				'setting_descriptionHeight' => [
					'text',
					'',
					$defaultData['setting_descriptionHeight'],
					__( 'Height of the description boxes', 'slideshow-jquery-image-gallery' ),
					'group' => __( 'Display', 'slideshow-jquery-image-gallery' ),
				],
				'setting_stretchImages'     => [
					'radio',
					'',
					$defaultData['setting_stretchImages'],
					__( 'Fit image into slide (stretching it)', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
				...
				'setting_showDescription'   => [
					'radio',
					'',
					$defaultData['setting_showDescription'],
					__( 'Show title and description', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
				...
				'setting_hideDescription'   => [
					'radio',
					'',
					$defaultData['setting_hideDescription'],
					__( 'Hide description box, it will pop up when a mouse hovers over the slide', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
				...
				'setting_play'              => [
					'radio',
					'',
					$defaultData['setting_play'],
					__( 'Automatically slide to the next slide', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
				...
				'setting_loop'              => [
					'radio',
					'',
					$defaultData['setting_loop'],
					__( 'Return to the beginning of the slideshow after last slide', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
				...
				'setting_controllable'      => [
					'radio',
					'',
					$defaultData['setting_controllable'],
					__( 'Activate buttons (so the user can scroll through the slides)', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
				...
				'setting_controlPanel'      => [
					'radio',
					'',
					$defaultData['setting_controlPanel'],
					__( 'Show control panel (play and pause button)', 'slideshow-jquery-image-gallery' ),
					[
						'true'  => $yes,
						'false' => $no,
					],
