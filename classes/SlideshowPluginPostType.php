...
class SlideshowPluginPostType {
	...
	public static function init() {
		add_action( 'init', [ __CLASS__, 'registerSlideshowPostType' ] );
		add_action( 'save_post', [ 'SlideshowPluginSlideshowSettingsHandler', 'save' ] );
		add_action( 'admin_enqueue_scripts', [ 'SlideshowPluginSlideInserter', 'localizeScript' ], 11 );

		add_action( 'admin_action_slideshow_jquery_image_gallery_duplicate_slideshow', [
			__CLASS__,
			'duplicateSlideshow',
		], 11 );

		add_filter( 'post_updated_messages', [ __CLASS__, 'alterSlideshowMessages' ] );
		add_filter( 'post_row_actions', [ __CLASS__, 'duplicateSlideshowActionLink' ], 10, 2 );
	}
	...
	public static function registerMetaBoxes() {
		...
		if (
			...
			add_action( 'admin_notices', [ __CLASS__, 'supportPluginMessage' ] );
		}
	}
	 ...
	 * @since 2.0.0
	 */
	public static function supportPluginMessage() {
	...
	public static function duplicateSlideshow() {
		...
		$errorRedirectURL = remove_query_arg( [ 'action', 'post', 'nonce' ] );

		// Check if nonce is correct and user has the correct privileges.
		if (
			! wp_verify_nonce( $nonce, 'duplicate-slideshow_' . $postID ) ||
			! current_user_can( 'slideshow-jquery-image-gallery-add-slideshows' ) ||
		...
		foreach ( $taxonomies as $taxonomy ) {
			$postTerms = wp_get_object_terms( $post->ID, $taxonomy, [ 'fields' => 'slugs' ] );
